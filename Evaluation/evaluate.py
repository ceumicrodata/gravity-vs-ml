...
result_files = list(str(x) for x in project_home.rglob('*') if 'prediction' in str(x) and '.csv' in str(x))
...
for result_file in result_files:
    try:
        ...
        measures = dict(zip(measure_names,(mae(predictions,targets), rmae(predictions,targets), mse(predictions, targets), rmse(predictions, targets), r2(predictions, targets))))
        measures['sha'] = sha
